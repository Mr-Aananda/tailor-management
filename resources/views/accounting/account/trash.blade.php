    ...
    <div class="container">
        ...
        <div class="print-none">
            <ul class="mt-2 nav nav-tabs">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="{{ route('accounts.trash') }}">Recycle Bin</a>
                </li>
            </ul>
        </div>
        <!-- container menu end -->
        ...
        <div class="border-0 card">
            ...
            <div class="p-0 mb-3 border-0 card-header d-md-flex align-items-center d-block">
                ...
                <!-- refresh -->
                <a href="{{ route('accounts.trash') }}" class="btn top-icon-button ms-auto" title="Refresh">
            ...
            <div class="p-0 card-body">
                <!-- search area -->
                <div class="collapse {{ request()->search ? 'show' : '' }}" id="account-search">
                    <div class="px-0 border-0 card card-body rounded-0">
                        <!-- search form -->
                        <form action="{{ route('accounts.trash') }}" method="GET">
                            <input type="hidden" name="search" value="1">

                            <div class="row gy-1 gx-3">
                                <div class="col-12 col-sm-6 col-lg-3">
                                    <label for="account-name" class="form-label">Account name</label>
                                    <input type="text" name="name" value="{{ request()->name }}" class="form-control" id="account-name" placeholder="Characters only">
                                </div>
                ...
                <div class="mb-3 d-block">
                    <form action="{{ route('accounts.trash') }}" method="POST">
                        @csrf
                                ...
                                <tbody>
                                    @forelse($records as $account)
                                        <tr>
                                            ...
                                            <td>{{ $account->name }}</td>
                                            <td class="text-end">
                                                <a href="{{ route('accounts.restore', $account->id) }}" class="btn table-small-button" title="Restore">
                                                    <i class="bi bi-bootstrap-reboot"></i>
                                                </a>

                                                <a href="{{ route('accounts.permanentDelete', $account->id) }}" onclick="return confirm('Are you sure, want to delete this data permanently?')" title="Permanent delete" class="btn table-small-button">
    ...
    @push('script')
        ...
        <script>
            ...
            let masterCheckbox = document.getElementById("check-all"),
                childCheckbox = document.querySelectorAll('[name="accounts[]"]');
