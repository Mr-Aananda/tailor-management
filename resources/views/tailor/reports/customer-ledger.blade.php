<x-app-layout>
    <x-slot name="title">Customer ledger</x-slot>
    ...
    <div class="container">
        ...
        <div class="border-0 card">
            ...
            <div class="p-0 border-0 card-header d-md-flex align-items-center d-block">
                <!-- page title -->
                <div class="mt-3 mb-2">
                    <h4 class="main-title">Customer ledger</h4>
                    <p><small>About {{ count($customer_ledgers) }} results found.</small></p>
                </div>
                ...
                <!-- refresh -->
                <a href="{{ route('ledger-report.customer') }}" class="btn top-icon-button print-none" title="Refresh">
                                    ...
                                    <div class="custom-select2">
                                        <select
                                            ...
                                            @foreach ($customers as $customer )
                                                <option {{ request()->search ? request()->customer_id == $customer->id ? 'selected' : '' : '' }} value="{{$customer->id}}">{{$customer->customer_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                    ...
                    <div class="mb-3 mt-5 table-responsive">
                     <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                                  <tr>
                                    <td >1</td>
                                    <th colspan="4" class="text-center">Opening balance</th>
                            ...
                            @forelse ($customer_ledgers as $ledger)
                                <tr>
                                    ...
                                    <td>{{ $ledger->date->format('d F Y') }}</td>
                                    {{-- <td>
                                        @if ($ledger->type === 'payment_details')
                                            {{ $ledger->order_no }}
                                        @else
                                            {{ $ledger->customerOrder->order_no ?? '' }}
                                        @endif
                                   ...
                                   <td  class="text-end">
                                        @if ($ledger->type === 'customer_order')
                                            {{ $ledger->total_paid }}

                                        {{-- Adjustment --}}
                                            @if ($ledger->adjustment != 0 )
                                                <span>
                                                    ({{$ledger->adjustment}})
                                                </span>
                                            @endif
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center">Customer ledger not available</td>
                                </tr>
